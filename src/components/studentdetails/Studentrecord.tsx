...
const Studentrecord = () => {
    ...
    console.log(termsmaster, "hhhh");
    console.log(academicYear, "yearid");
    useEffect(() => {}, []);
    const paginate = [
        { text: "5", value: 5 },
        { text: "10", value: 10 },
    ...
    const col: any = [
        {
            ...
            formatter: (cell: any, row: any, rowIndex: any, formatExtraData: any) => {
                return (
                    <>
                        <Link to={`/StudentprofileSearch/${row.student_admissions_id}`}>{row.student_name}</Link>
                    </>
                );
            },
            sort: true,
    ...
    const callStudentData = () => {
        ...
        if (academicYear && academicYear.length > 0) {
            if (searchBy && searchBy.length > 0) {
                ...
                } else if (academicYear && academicYear.length > 0 && gradea && gradea.length > 0 && termsmaster && termsmaster.length > 0) {
                    getAccessToken();
                    axios
                        .post(`${baseUrl}autoSearch`, {
                            academic_year: academicYear,
                ...
                } else if (academicYear && academicYear.length > 0 && section && section.length > 0 && termsmaster && termsmaster.length > 0) {
                    getAccessToken();
                    axios
                        .post(`${baseUrl}autoSearch`, {
                            academic_year: academicYear,
                ...
                } else if (academicYear && academicYear.length > 0 && gradea && gradea.length > 0 && section && section.length > 0) {
                    getAccessToken();
                    axios
                        .post(`${baseUrl}autoSearch`, {
                            academic_year: academicYear,
                                        ...
                                        <Row>
                                            ...
                                            {Autosearch && Autosearch.text ? (
                                                ...
                                                <>
                                                    ...
                                                    <Col md={2}>
                                                        <Form.Select aria-label="Default select example" onChange={(e: any) => setsection(e.target.value)}>
