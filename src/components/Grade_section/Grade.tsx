...
const Grade = () => {
  ...
  const [statuspagination,setPagination] = useState<any>({});
  ...
  const dataSearch: any =
    statusList.length &&
    statusList.sort().filter((data: any) => {
  ...
  const getAllGradeSectionData = () => {
    axios
      .get(`${baseUrl}grade_section/show_all?page=1&per_page=${perPage}`)
  ...
  const handleSubmit = () => {
    if (
      ...
      clickedGrade.forEach((element: any) => {
        ...
        getAccessToken();
        axios
          .post(`${baseUrl}grade_section/create`, sendData)
          .then((res: any) => {
  ...
  let active = statuspagination.page;
let items = [];
for (let number = 1; number <= 5; number++) {
  ...
  console.log(statuspagination.per_page);
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  setAcademic_year_data(
                                    allAcademicYear[0].academic_year
                                  );
                                }}
                              >
                                Cancel
